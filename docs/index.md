# CFDI Express

CFDI Express es una [aplicación de Shopify](https://apps.shopify.com/cfdi-express) para generar Certificados Fiscales Digitales por Internet (CFDIs) o facturas mexicanas en todo el ecosistema de Shopify.

## Instalación

1. Para poder instalar el app de Shopify en tu tienda entra en la siguiente liga.
[App de Shopify de CFDI Express](https://apps.shopify.com/cfdi-express)

2. Dentro encontraras un botón para instalar la aplicación cómo se muestra en la imagen a continuación.
![Botón de instalación de CFDI Express](https://cdn.shopify.com/s/files/1/0804/5540/1763/files/Acr_2025-07-17_at_16.10.16_2x_dde3f921-2e2f-4540-aa61-22040bde1ae3.png?v=1752790261)

3. Al hacer click en el botón serás redireccionado al login de Shopify, inicia sesión con tu usuario y en la tienda donde quieras instalar el app.

4. Acepta los permisos requeridos para instalar el app.
![Pantalla de permisos Shopify CFDI Express](https://cdn.shopify.com/s/files/1/0804/5540/1763/files/Acr_2025-07-17_at_16.16.12_2x_a2a66b03-e5c6-4614-9f7f-d5b0de9bba35.png?v=1752790590)

5. Acepta el cargo de la aplicación.
![Pantalla de Cargo](https://cdn.shopify.com/s/files/1/0804/5540/1763/files/Acr_2025-07-17_at_16.16.44_2x_67e50e78-3089-49d6-966b-2a1565bf9151.png?v=1752790623)

6. Listo, ya puedes comenzar a configurar CFDI Express.

### Funcionalidades

- [Onboarding y configuración](/docs/onboarding)
- [Configuraciones](/docs/configuraciones)
- [Planes y Precios](/docs/planes-y-precios)
- [Lista de Órdenes y Estatus](/docs/ordenes-estatus)
- [Facturación desde el administrador de Shopify](/docs/facturacion-desde-admin-shopify)
- [Cancelación de CFDI y Acuse de Cancelación](/docs/cancelacion-y-acuse)
- [Complementos de Pago (PPD)](/docs/complementos-de-pago)
- [Códigos del SAT por producto y IEPS](/docs/codigos-sat-producto)
- [Carga Masiva de Productos (CSV)](/docs/carga-masiva-productos-csv)
- [Buscador de Códigos del SAT](/docs/buscador-de-codigos-sat)
- [Facturación desde el Checkout/Thank You Page](/docs/facturacion-checkout-thank-you-page)
- [Facturación desde la página de estado de orden](/docs/facturacion-pagina-estado-de-orden)
- [Formulario de Facturación (Theme App Block)](/docs/formulario-de-facturacion-theme-block)
- [Facturación desde el POS (Punto de Venta) de Shopify](/docs/facturacion-pos-punto-de-venta-shopify)
- [Facturación al Público en General (Manual y Automática)](/docs/facturacion-al-publico-en-general-manual-y-automatica)
- [Facturación al Extranjero (RFC XEXX010101000)](/docs/facturacion-al-extranjero)
- [Uso de Flow para automatizaciones con CFDI Express](/docs/uso-shopify-flow-automatizaciones-cfdi-express)
- [Modo Flow en CFDI Express para usar tu propio PAC de Facturación](/docs/modo-flow-cfdi-express-para-usar-tu-propio-pac-de-facturacion)
- [Reportes de CFDIs y Complementos](/docs/reportes)
- [Notificaciones por Correo](/docs/notificaciones-por-correo)
- [Acción de Impresión de CFDI en el Admin](/docs/accion-de-impresion-admin)
